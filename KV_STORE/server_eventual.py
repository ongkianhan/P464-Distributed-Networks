...
def get_key(key):  # key is a string
    file1 = open("database_eventual.txt", "r")
    flag = 0
    index = 0
    keyLength = len(key)
    ...
    if flag == 0:
        ...
        readThis = open("database_eventual.txt")
...
def set_key(key, value):  # key and value are strings
    file1 = open('database_eventual.txt', 'a+')
    flag = 0
    index = 0
    keyLength = len(key)
...
def update_database():
    ...
    file_exists_replica2 = os.path.isfile("database_eventual.txt")
    if file_exists_replica2 and file_exists_master:
        file_r2 = open("database_eventual.txt", "w")
...
def start(port_number):
    global PORT
    PORT = port_number
    file_exists = os.path.isfile("database_eventual_r1.txt")
    print("Server_eventual REPLICA 1 has connected to Server_eventual PRIMARY...")
    if file_exists:
        pass
    else:
        file = open("database_eventual.txt", "w")
        file.close()
    print(f"[LISTENING] Server(EVENTUAL, REPLICA 1) is listening on server: {SERVER}, port: {PORT}")
    server.listen()
    while True:
