...
def get_key(key):  # key is a string
    file1 = open("database_sequential.txt", "r")
    flag = 0
    index = 0
    keyLength = len(key)
    ...
    if flag == 0:
        ...
        readThis = open("database_sequential.txt")
...
def set_key(key, value):  # key and value are strings
    file1 = open('database_sequential.txt', 'a+')
    flag = 0
    index = 0
    keyLength = len(key)
...
def start(port_number):
    ...
    PORT = port_number
    server.listen()
    file_exists = os.path.isfile("database_sequential.txt")
    if file_exists:
        pass
    else:
        file = open("database_sequential.txt", "w")
        file.close()
    print(f"[LISTENING] Server(sequential) is listening on server: {SERVER}, port: {PORT}")
    while True:
